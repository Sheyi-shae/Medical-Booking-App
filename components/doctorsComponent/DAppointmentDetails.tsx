...
export default function DAppointmentDetails({id}:IdString) {
  ...
  return (
    <div className='w-full bg-slate-50 flex flex-col gap-2 px-3 shadow-md'>
      {loading ? (
        ...
        <Card className='text-slate-800  '>
          ...
          <div className='flex space-x-2 p-3 '>
            <div className='' >
            <Image src={'/images/main.webp'} height={200} width={200} 
            className='rounded-full w-12 h-12 ' alt='dp' /></div>
            <div className='flex flex-col'>
                <h3 className='text-base lg:text-lg '>{appDetails?.patientFullName}</h3>
                <span className='text-xs gap-1 flex text-slate-600 mb-1'><Mail size={14}/> {appDetails?.patientEmail}</span>
            </div>
     ...
     {appDetails?.status==='PENDING' &&    <div className='flex justify-end gap-2'>
          <DeclineEvent id={id}/>
        
          <Button className="text-white bg-gradient-to-r from-cyan-500
       to-blue-500 hover:bg-gradient-to-bl focus:ring-4 focus:outline-none
        focus:ring-cyan-300 dark:focus:ring-cyan-800 font-medium rounded-sm 
         text-sm px-5 py-2.5 text-center me-2 mb-2" onClick={acceptApp} >Accept</Button>
